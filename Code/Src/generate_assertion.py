...
def generate_test_transactions_and_assertions(contract_name, topic2event, abi, selector2func, args):
    ...
    for test_case in os.listdir(test_case_folder):
        ...
        if not os.path.exists(os.path.join(test_case_folder, test_case.split('_')[0]+'_assertion.json')):
            with open(os.path.join(test_case_folder, test_case.split('_')[0]+'_assertion.json'), 'w+') as fo:
